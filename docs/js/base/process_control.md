...
```
### break关键字
case的穿透性：当某个**case**匹配成功的话，它后面的所有**case**都跟着匹配成功，即使后面的**case**表达式不符合条件也会匹配，这就是**case**的穿透性。

**break**关键字用于中断穿透，仅匹配到当前的**case**为止。
...
与`while`的区别是，`do-while`会先执行一次循环体，再判断表达式来决定是否继续执行循环体，也就是说，它能保证语句至少可以执行一次。
```js
do {
...
可以发现，初始化表达式，仅在第一轮循环时执行，从第二轮循环开始就不再执行了。

其实就像`while`的变量初始化是在`while`结构外部定义的一样，如果在循环结构内部定义，那么重复执行初始化表达式，循环一直满足条件一直执行，就造成死循环了，所以这种设计是合理的。
```js
for (var i = 0; i < 3; i++) {
  console.log(i);
}
...
```
也可以不写，因为分号`;`本身就代表了一个简短表达式。
::: warning
但是注意，不写的话，表达式的值就是`true`，将会造成死循环。
:::
...
## break & continue
`break`用于永远退出分支，退出循环等等，特别是针对于死循环，`break`很管用。

但是`break`有一个注意点：不能单独使用在`if`语句中，如果该`if`语句存在于`switch`、`while`、`for`之中，那么是可以使用的。
```js
var num = 100;
switch (num) {
  case 100:
